...
DATES=2024040100  #2024031418  #2024072400 #2021032100
DATEE=2024041000  #2024033118  #2024082100  #2022123118 #2022123118 
gfs_ver=v16
...
export yy_m6=`echo ${cdatem6} | cut -c 1-4`
export mm_m6=`echo ${cdatem6} | cut -c 5-6`
export dd_m6=`echo ${cdatem6} | cut -c 7-8`
export hh_m6=`echo ${cdatem6} | cut -c 9-10`
...
EXTRACT_DIR=${TMPDIR}/${gfs_ver}-2/${cdate}
...
if [ $EXTRACT_DATA == yes ]; then
  ...
  case $gfs_ver in
    ...
    v16)
      DATAH=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_${CDUMP} \
       -o ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} -e ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} ${CURDIR}/get_v16.gdas.sh ${CDUMP})
      DEPEND="-d afterok:$DATAH"
      if [ "$CDUMP" = "gdas" ] ; then
        DATA1=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp1 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp1 -e ${EXTRACT_DIR}/log.data.${cdate}.grp1 ${CURDIR}/get_v16.gdas.sh grp1)
        DATA2=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp2 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp2 -e ${EXTRACT_DIR}/log.data.${cdate}.grp2 ${CURDIR}/get_v16.gdas.sh grp2)
        DATA3=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp3 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp3 -e ${EXTRACT_DIR}/log.data.${cdate}.grp3 ${CURDIR}/get_v16.gdas.sh grp3)
        DATA4=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp4 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp4 -e ${EXTRACT_DIR}/log.data.${cdate}.grp4 ${CURDIR}/get_v16.gdas.sh grp4)
        DATA5=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp5 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp5 -e ${EXTRACT_DIR}/log.data.${cdate}.grp5 ${CURDIR}/get_v16.gdas.sh grp5)
        DATA6=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp6 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp6 -e ${EXTRACT_DIR}/log.data.${cdate}.grp6 ${CURDIR}/get_v16.gdas.sh grp6)
        DATA7=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp7 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp7 -e ${EXTRACT_DIR}/log.data.${cdate}.grp7 ${CURDIR}/get_v16.gdas.sh grp7)
        DATA8=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp8 \
         -o ${EXTRACT_DIR}/log.data.${cdate}.grp8 -e ${EXTRACT_DIR}/log.data.${cdate}.grp8 ${CURDIR}/get_v16.gdas.sh grp8)
        DEPEND="-d afterok:$DATAH:$DATA1:$DATA2:$DATA3:$DATA4:$DATA5:$DATA6:$DATA7:$DATA8"
      fi
      ;;
 esac
...
heraenkf_m6dir=${EXTRACT_DIR}/enkfgdas.${yy_m6}${mm_m6}${dd_m6}/${hh_m6}
hpssdir=${HPSSDIR}/${gfs_ver}/${yy}${mm}
hsi "mkdir -p \${hpssdir}"

if [ -f \${heracntldir}/atmos/gdas.t${hh}z.atmanl.nc ] && [ -f \${heracntldir}/atmos/gdas.t${hh}z.sfcanl.nc ]; then
    cd \${heracntldir}
    ...
    echo "-f \${heracntldir}/atmos/gdas.t${hh}z.atmanl.nc does not exist and exist"
    exit 1
fi

if [ ${cdate} -eq 2021032100 ]; then
    nens1=\`ls \${heraenkf_m6dir}/mem???/gdas.t${hh_m6}z.atmf006.nemsio | wc -l\`
    nens2=\`ls \${heraenkf_m6dir}/mem???/gdas.t${hh_m6}z.sfcf006.nemsio | wc -l\`
    if [ \${nens1} -eq 80 ] && [ \${nens2} -eq 80 ] ; then
        cd \${heraenkf_m6dir}
        htar -cv -f \${hpssdir}/enkfgdas.${cdatem6}.tar *
        err=\$?
        if [ \${err} != '0' ]; then
            echo "HTAR enkf failed at \${cdatem6} and exit."
            exit \${err}
        else
            echo "HTAR enkf succeeded at ${cdatem6}."
...
nens1=\`ls \${heraenkfdir}/atmos/mem???/gdas.t${hh}z.ratminc.nc | wc -l\`
nens2=\`ls \${heraenkfdir}/atmos/mem???/gdas.t${hh}z.atmf006.nc | wc -l\`
nens3=\`ls \${heraenkfdir}/atmos/mem???/gdas.t${hh}z.sfcf006.nc | wc -l\`
...
sbatch --parsable --partition=service --ntasks=1  -t $WALLT -A $PROJECT_CODE -q $QUEUE -J ${gfs_ver}-2_${cdate} \
