...
DATES=2018060612 #2017100100
DATEE=2018060612 #2019061118
gfs_ver=v14
  ...
  case $gfs_ver in
    v14)
      DATAH=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_${CDUMP} \
       -o ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} -e ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} ${CURDIR}/get_v14.gdas.sh ${CDUMP})
      DEPEND="-d afterok:$DATAH"
      if [ "$CDUMP" = "gdas" ] ; then
        DATA1=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_enkf \
         -o ${EXTRACT_DIR}/log.data.${cdate}.enkf -e ${EXTRACT_DIR}/log.data.${cdate}.enkf ${CURDIR}/get_v14.gdas.sh enkf)
        DEPEND="-d afterok:$DATAH:$DATA1"
      fi
      ;;
    v15)
      DATAH=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_${CDUMP} \
       -o ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} -e ${EXTRACT_DIR}/log.data.${cdate}.${CDUMP} ./get_v15.gdas.sh ${CDUMP})
      DATA1=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp1 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp1 -e ${EXTRACT_DIR}/log.data.${cdate}.grp1 ./get_v15.gdas.sh grp1)
      DATA2=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp2 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp2 -e ${EXTRACT_DIR}/log.data.${cdate}.grp2 ./get_v15.gdas.sh grp2)
      DATA3=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp3 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp3 -e ${EXTRACT_DIR}/log.data.${cdate}.grp3 ./get_v15.gdas.sh grp3)
      DATA4=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp4 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp4 -e ${EXTRACT_DIR}/log.data.${cdate}.grp4 ./get_v15.gdas.sh grp4)
      DATA5=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp5 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp5 -e ${EXTRACT_DIR}/log.data.${cdate}.grp5 ./get_v15.gdas.sh grp5)
      DATA6=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp6 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp6 -e ${EXTRACT_DIR}/log.data.${cdate}.grp6 ./get_v15.gdas.sh grp6)
      DATA7=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp7 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp7 -e ${EXTRACT_DIR}/log.data.${cdate}.grp7 ./get_v15.gdas.sh grp7)
      DATA8=$(sbatch --parsable --partition=service --ntasks=1 --mem=$MEM -t $WALLT -A $PROJECT_CODE -q $QUEUE -J get_grp8 \
       -o ${EXTRACT_DIR}/log.data.${cdate}.grp8 -e ${EXTRACT_DIR}/log.data.${cdate}.grp8 ./get_v15.gdas.sh grp8)
